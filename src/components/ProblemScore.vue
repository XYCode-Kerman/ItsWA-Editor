<template>
    <div>
        <div @click="show_detail = !show_detail" class="cursor-pointer">{{ problem_sum_score(problems_result) }}</div>

        <div v-if="show_detail" class="divider my-2">测试点详情</div>

        <div v-if="show_detail" class="grid grid-cols-2">
            <div v-for="item, index in problems_result" :key="index" class="tooltip" :data-tip="'得分：' + item.score">
                测试点 {{ index }}：
...
const props = defineProps(['problems_result'])
const problems_result = props.problems_result as ProblemResult[]
